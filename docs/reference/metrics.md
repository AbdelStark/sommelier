...
The metric names and `MetricValue` shape are part of the artifact contract: they appear under `metrics` in every [evaluation report](artifacts.md), and the [comparison gate](../concepts/determinism.md) refuses to compare reports whose metric names differ. Deltas in the comparison report are plain value differences, adapter minus base.
