...
The successful run's GPU bill was about eight dollars according to the maintainer's private billing console; it is not part of the public checksummed run evidence. My mistakes along the way (failed runs, plus a serving crash-loop I let spin for three hours) were observed to cost about as much again. The [adapter](https://huggingface.co/abdelstark/llama-3.1-nemotron-nano-8b-xlam-tool-calling-lora), [exact train/val/test splits](https://huggingface.co/datasets/abdelstark/sommelier-xlam-single-call-splits), and aggregate evaluation reports are on Hugging Face. The public reports expose the metric counts and digests behind the rates above, but the published repository does not include the retained raw generations needed for independent row-wise rescoring.
