...
**The bilingual chain on real fixture files.** A hand-written French fixture (`tests/fixtures/preparation_rows.fr.jsonl`) pairs the 30-row English raw fixture, with deliberate edge rows: a pair naming a root example that does not exist, mutated tools, mutated answers, a duplicate pairing, a unicode-heavy query, and an untranslated pair identical to its English source. A fresh clone with no network runs the whole bilingual path over these files: prepare (split inheritance and every pairing drop reason asserted by exact count), fixture formatting, slice-aware evaluation with stub generators shaped to produce a synthetic transfer gap, and the gated comparison with its language gaps section. The translation tool itself is covered separately with an injected fake model, including retries, resume, and every audit rejection.
