...
\begin{itemize}
  ...
  \item \textbf{Position: sovereignty is operational, and it now has a price tag} (Section~\ref{sec:sovereignty}). Sovereignty over a tool-calling model, for one task, cost us about eight dollars of compute, three hours of GPU time, and a pipeline you can read in an afternoon. A second turn that closed a language gap cost roughly sixteen dollars more. We argue this reframes sovereign AI~\citep{singh2025sovereignai,srivastava2024digitalsovereignty,nvidia2024sovereignai} from infrastructure policy to weekend engineering, and we measure a dependency that policy discussions rarely quantify: language. The base model pays 4.2 points of full-call exactness on French input; that tax is baked into a model you cannot retrain.
...
Observed cost in \sommelier{} is evidence, not an estimate: the run records report cost as unavailable because the provider exposed no billing data to the run itself. From the maintainer's billing console, outside the run record, turn one billed about eight dollars for roughly 3.7 GPU hours end to end; turn two, at the same observed rate over about seven GPU hours, is roughly sixteen dollars. Failed attempts along the way (including a serving crash-loop, Section~\ref{sec:recipes}) cost about as much again as turn one, and we report that too, because the price of the capability includes the mistakes.
...
\textbf{The flywheel's own output closes the rest.} With the 13{,}113 machine-translated pairs mixed in, French matches English on every metric to within a third of a point ($+0.3$ points on full-call exact match). The training data for this turn cost no annotation: it was manufactured from turn one's published artifacts by an open, pinned translator under a byte-identity contract, with every rejected pair counted (Section~\ref{sec:setup}). The cost, reported rather than hidden: the v2 adapter's English slice sits 0.3 to 0.8 points below the v1 reference depending on the metric (full-call 0.8700 against 0.8740). At $n{=}1{,}000$ this is within one standard error, and with one seed we cannot decide between noise and a small capacity price for bilingual coverage. We state it in those terms because a number that does not admit its uncertainty is an advertisement.
...
We set out to answer a question a thousand teams are asking: can a small open model, post-trained cheaply on your own schemas, carry the tool-calling load of an agentic product, and can you prove it with numbers instead of vibes? Two turns of an auditable flywheel later, the answer is concrete. Three GPU hours and eight dollars bought a 16.9-point lift in full-call exactness with structural validity saturated; sixteen more dollars, spent on data the flywheel manufactured from its own published artifacts, bought French parity to within a third of a point. The model learned the sommelier's job quickly. Building the instruments that make the measurement trustworthy, the digests, the gates, the counted drops, the parser that refuses to help, took most of the effort, and that ratio is, we suspect, the honest shape of applied AI work.

The position we hope travels: turn your own flywheel, and make every turn auditable. The first property makes you sovereign; the second keeps you honest, and the loop compounds only if it is both. Everything needed to check us, or to start your own wheel, is public: the pipeline, the exact splits, the adapters, the raw generations, and the reports that refuse to exist unless the comparison was fair~\citep{bakhta2026sommelier}. Steal the comparison gate and the boundary-proving collator first; argue with the parser; and if you disagree with a number, the artifacts are how you will show it.

\begin{ack}
This work builds on NVIDIA's open Nemotron stack (weights, tooling, and published data recipes) and on the Hugging Face open-source ecosystem. The paper was prepared with AI writing and analysis assistance; all numbers were verified against the published run artifacts.
\end{ack}
...
Full digests, per-slice evaluation reports, runtime metadata, and raw generations are published under \texttt{reports/} in the adapter repositories on the Hugging Face Hub:
\begin{itemize}
...
Per-prompt outcome transitions between base and v1 adapter on the 1{,}000 English prompts. Outcomes: exact (full-call exact match), args\_mismatch (right name, wrong arguments), name\_mismatch, and the three parse-failure statuses.
...
Local validation needs no GPU and no accounts: \texttt{uv sync --extra dev}, then \texttt{ruff}, \texttt{mypy}, \texttt{pytest} (390 tests), config validation, and a fixture pipeline slice. Publication requires \texttt{sommelier release preflight} with the base-model license acknowledged by exact identifier. Expect metrics within noise of the published runs, verified against the same splits and digests; bit-identical generations across different GPUs and driver stacks are not guaranteed.
